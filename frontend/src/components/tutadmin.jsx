...
const TutAdmin = () => {
    ...
    useEffect(() => {
        const myTutLink = `http://192.168.1.5:5555/tutorial`;
    ...
    const handleUpdateRemarks = async (id) => {
        const updateRemarksLink = `http://192.168.1.5:5555/tutorial/${id}/remarks`;
        try {
            const updatedTuts = await updateRemarks(updateRemarksLink, id, newRemarks);

            setTutorial((prevTuts) =>
                prevTuts.map((tut) =>
                    tut._id === id ? { ...tut, remarks: updatedTuts.remarks } : tut
                )
            );
                                ...
                                <tbody>
                                    {tutorial.map((cons, index) => (
                                        <tr key={index} className='odd:bg-customWhite even:bg-customGray'>
                                            ...
                                            <td className='p-2 sm:p-4'>{cons.request_date}</td>
